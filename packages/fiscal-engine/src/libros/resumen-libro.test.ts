...
describe('resumirLibro — resumen del Libro de Ventas/Compras (Reglamento IVA arts. 70–78)', () => {
  it('suma facturas por alícuota y separa exentas, exoneradas y exportación', () => {
    const filas: FilaImpuestoLibro[] = [
      ...
      { alicuotaCodigo: 'EXONERADO', alicuotaTasa: '0', base: '100.00', monto: '0', factor: 1 },
    ...
    expect(r.baseExonerada).toBe('100.00');
    expect(r.baseExportacion).toBe('400.00');
    expect(r.baseTotal).toBe('2500.00');
    expect(r.totalConIva).toBe('2756.00');
  });
